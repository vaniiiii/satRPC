...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> Result<Binary, ContractError> {
    match msg {
        ...
        QueryMsg::GetOperatorMaxScore { operator } => query_operator_max_score(deps, operator),
    }
}
...
fn query_operator_max_score(deps: Deps, operator: Addr) -> Result<Binary, ContractError> {
    let score = OPERATOR_MAX_SCORE.may_load(deps.storage, operator)?;
...
mod tests {
    ...
    fn query_operator_max_score() {
        ...
        // Query the operator's score
        let operator_addr = Addr::unchecked("operator");
        let query_msg = QueryMsg::GetOperatorMaxScore {
            operator: operator_addr,
        };
        let res = query(deps.as_ref(), env, query_msg).unwrap();
